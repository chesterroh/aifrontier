...
![A tweet thread by Noam Brown summarizing remarks from Ilya Sutskever on a podcast. Under the title 'Ilya × Noam + Andrej,' it says Ilya mentioned that scaling will keep improving performance for now, but that something important will still be missing, along with a captured tweet summarizing AGI timeline predictions from researchers including Hassabis, Chollet, Sam Altman, Yann LeCun, and Ilya.](/episodes/ep78/notion_01.png)
...
![A scene from the Dwarkesh Patel interview showing the two participants seated facing each other in conversation. The subtitle on the left reads 'How will SSI make money?' and the subtitle on the right reads 'Right now, we just focus on the research, and then the answer to that question will reveal itself. I think there will be lots of possible answers.'](/episodes/ep78/notion_03.png)
...
![Slide 1 of 12 generated by Claude Opus 4.5. A slide-style image featuring the large quote 'Isn't it amazing that all of this is actually happening?' along with the description 'The era of the first truly science-fiction-like AI. We adapt quickly to being amazed by new things.'](/episodes/ep78/notion_05.png)
...
![Left: Anthropic's Claude Opus 4.5 launch page ('Meet Opus 4.5, built for specialized work'). Right: the Claude Code welcome screen ('Your agentic coding partner that works directly in your codebase') and a code editing interface.](/episodes/ep78/notion_09.png)
...
![Anthropic engineering blog post titled 'Effective harnesses for long-running agents.' It explains that agents still struggle to operate across multiple context windows, and that the team built more effective long-running agent harnesses inspired by human engineers. Published on November 26, 2025.](/episodes/ep78/notion_11.png)
