...
![METR benchmark chart: a scatter plot showing task duration on the x-axis, by release year, for various LLMs on software engineering tasks that they can complete with 50% probability. It shows an exponential upward curve from GPT-2 in 2020 to Claude Opus 4.5 in 2026 (about 4 hours 49 minutes), with Claude Opus 4.5 marked at the highest value in the upper-right of the graph.](/episodes/ep82/notion_01.png)
...
![A slide composed of three sections. At the upper left is the large title “Why do SKILLs work so well?”, and at the upper right is the subtitle “A pipeline that creates pipelines that generate useful byproducts” with a chain-shaped diagram. At the bottom, two screenshots are arranged side by side: an example Domain Priming skill screen from the Claude interface (including code and explanation), and a table about AI self-improvement policies (including items such as name, methodology, and scope).](/episodes/ep82/notion_02.png)
...
![A slide with large black text on a yellow background reading “From f(x) and g(x) to f(g(x)):” and, below it in smaller text, the explanatory sentence “LLMs learn new skills in RL by combining existing skills.”](/episodes/ep82/notion_04.png)
...
![A typography slide on a beige background with the large Korean calligraphy-style title “Step-by-Step Transformer,” with the abbreviation “MoE” also shown in red on the right.](/episodes/ep82/notion_05.png)
...
![A slide with the title “The meaning of nonlinear activation functions” on the left, followed by Korean mathematical explanations showing that in an MLP, without nonlinear activations, multiple layers collapse into a single linear transformation. On the right, Python code using NumPy verifies the linear-combination equivalence of a two-layer MLP, alongside the execution result confirming that the two arrays are identical.](/episodes/ep82/notion_08.png)
